...
当前项目阶段是 SwiftUI App 骨架已完成；2026-05-16 经 James 确认采用
P0→P4 执行策略（先定契约 → 最薄垂直闭环 → 再扩重路径 → 收敛硬化）。
P0 `TC 核心契约层` 已完成，下一步进入 P1 起点 `T03 本地配置与密钥边界`。
具体拆分与状态以 `project-board.md` 为准。
目录中主要资产为：
