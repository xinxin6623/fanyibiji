...
<!-- 由 /project-init --complete 于 2026-06-01 追加：项目通用守则 4 段，与上方项目特定约束互补 -->

## 11. 三件套文档维护节奏

本项目根目录三份入口文档**不是平权的**，更新频率和触发条件不同，按错节奏会污染 signal：

- **AGENTS.md（活文档）**：**随时更新**。工作中发现新的项目惯例、踩坑、约束、决策，立刻加进来。这是"项目实时手册"——下一个 agent 接手就少踩一次坑。边干边写，不要等收尾。
- **INDEX.md（结构快照）**：**阶段性更新**。只在顶层目录 / 子模块结构变化（新增 / 重命名 / 删除 / 归档）时同步。不要每改一个文件就动。
- **CHANGELOG.md（演绎记录）**：**阶段性更新**。每个任务 / 功能 / 修复**告一段落时**追加一条，不是每个 edit 一条。按 CHANGELOG 文件顶部强标签格式写。只记摘要 + Why + 详细记录指针，不贴 diff。
...
**注意**：本项目原 §8 "新会话接续与文档维护" 讲的是**已有文档之间的优先级**（AGENTS / relay-task / project-board / prd-mvp），本节讲的是**根目录三件套的更新节奏**——两者不冲突，并行参照：交接靠 §8 优先级链；动了什么靠 §11 节奏 + CHANGELOG。

## 12. 子项目嵌套（预留规范）

当前 fanyibiji 项目根目录下**没有**子项目（PersonalAgent/ 等是源码目录不是带三件套的子项目）。本段是**预留规范**，如未来某个子目录自己跑 `/project-init` 起了三件套：
...
| 子目录 | 用途 |
|---|---|
| `PersonalAgent/` | App 主代码（Swift） |
| `PersonalAgentTests/` | XCTest 单测 |
| `PersonalAgent.xcodeproj/` | Xcode 工程文件（显式引用式 pbxproj） |
| `tools/` | 辅助构建工具（含 cm6-build） |
| `archive/` 或 `不加载/` | 归档区，不参与构建（暂未启用） |

新增顶层目录前先考虑能否归入已有目录；新增 Swift 源文件需手动补 pbxproj 4 处（PBXBuildFile / PBXFileReference / PBXGroup children / PBXSourcesBuildPhase）。

## 14. 语言规则（与 ~/.claude/CLAUDE.md 全局规则对齐）

- 解释性内容、架构决策、注意事项、提交信息、PR 描述 → **中文**
- 代码标识符（变量名、函数名、类名、目录名）→ 英文
- 行内 AI 参考注释可英文，James 无需阅读
- 与项目所有者对话 → **中文**（除非对方先用英文）
